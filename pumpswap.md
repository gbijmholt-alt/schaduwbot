# PumpSwap-dekking — 2026-09-14 20:19 UTC
...
Totaal open (SOL erin min eruit op de curve): **174514 SOL** over 75781 posities. Hiervan gecheckt: 3033 posities (58825 SOL).

| status nu | posities | open SOL |
|---|---|---|
| verkocht | 2931 | 57654.6 |
...
Poolprijzen: geen_antwoord: 26, goedgekeurd: 106, prijs_onwaarschijnlijk: 1158. Mediane verhouding met de laatste curveprijs: 0.32×. Afgekeurde prijzen tellen niet mee in de restwaarde: de grootste tokenhouder is niet altijd de pool, en bij een gewone wallet met veel WSOL rolt er een onzinprijs uit.

| groep | status | posities | open SOL |
|---|---|---|---|
| gevolgd | deels_verkocht | 3 | 2.5 |
| gevolgd | nog_in_bezit | 5 | 19.7 |
| gevolgd | verkocht | 286 | 770.1 |
...
Programma `pAMMBay6oceH9fJKBRHGP5D4bD4sWpmSwMn52FMfXEA`. 3513 transacties opgehaald, 1237 bruikbaar (één memecoin-mint, bedragen uit pre/post-balansen af te leiden). Eis om een layout vast te stellen: match ≥ 95% over ≥ 50 voorbeelden.

| discriminator | naam | waar | n | tokens | lamports | mint | pool | user | herkenning | vastgesteld |
|---|---|---|---|---|---|---|---|---|---|---|
| `67f4521f2cf57777` | BuyEvent | inner_cpi+log | 821 | @8 (96%) | @96 (96%) | – | @112 (100%) | @144 (82%) | pool | ja |
| `3e2f370aa503dc2a` | SellEvent | inner_cpi+log | 443 | @8 (94%) | @376 (98%) | – | @112 (100%) | @144 (87%) | pool | nee |
| `c62e1552b4d9e870` | ? | inner | 53 | – | @7 (2%) | – | – | – | – | nee |
| `33e685a4017f83ad` | ? | inner | 49 | @0 (92%) | – | – | – | – | – | nee |
...
**Layout vastgelegd** in `data/pumpswap_layout.json`: BuyEvent (match 96%, n=821, herkenning via pool)
...
**Veld vastgesteld op offset 43** (278 van 278 pools, 100%; accountlengte 301 bytes). Daarmee vraagt de analyse bij de keten op welke pool bij een mint hoort, en leest daarna de twee vaten van die pool. Dat vervangt de oude noodgreep 'de grootste tokenhouder is vermoedelijk de pool'.
...
| 33.5 | 12.0954 | 96.4543 | 593,337,626 | 2.0385358132859158e-08 | 4.1088017215791354e-07 | 0.04961 |

| route | prijzen | p10 | mediaan | p90 | (poolprijs ÷ laatste curveprijs)
|---|---|---|---|---|---|
| pool_uit_programma | 1264 | 0.00047 | 0.00597 | 58.26677 | |

Koersen van gemigreerde tokens opgehaald voor de afloopanalyse: 95 deze run, 1468 te gaan, 25 calls mislukt.

Koersen per route: pool_uit_programma/geen_antwoord: 26, pool_uit_programma/goedgekeurd: 106, pool_uit_programma/prijs_onwaarschijnlijk: 1158
