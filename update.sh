...
SUM=$(cat /opt/schaduwbot/wallet_analysis.py /opt/schaduwbot/ledger.py /opt/schaduwbot/video_replay.py /opt/schaduwbot/pumpswap.py /opt/schaduwbot/hypotheses.py /opt/schaduwbot/lotgevallen.py 2>/dev/null | sha1sum | cut -c1-12)
...
if [ -f /opt/schaduwbot/wallet_analysis.py ] && ! systemctl is-active --quiet schaduwbot-wallets; then
  ...
  if [ "$LAST_SUM" != "$SUM" ] || [ "$AGE" -gt 7200 ]; then
    ...
    if systemd-run --unit=schaduwbot-wallets --collect -p RuntimeMaxSec=5400 /bin/bash -c \
         'cd /opt/schaduwbot && set -a && . ./.env && set +a && for s in ledger.py hypotheses.py pumpswap.py lotgevallen.py video_replay.py wallet_analysis.py; do [ -f "$s" ] && nice -n 19 ionice -c3 .venv/bin/python "$s" >> reports/wallets.log 2>&1; done'; then
